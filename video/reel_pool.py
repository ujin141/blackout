...
아니라 장소 소개고, 장소 소개를 보고 들어오는 사람은 없다.
    ...
    DJ 두 컷 얼굴 하나 · 손 하나. 손만 보여 줬더니 "DJ 여자도 보여 달라" 는
            지적이 왔다 — 맞다. 크루가 튼다는 걸 말하는 건 손이 아니라 얼굴이다
...
# 자를 여백이 없기 때문이다.
CUTS = [
    ('P1023235',  1.4, 6, 0.50, 0.50, 1.05, '풀 가득 · 튜브 (훅)'),
    ('P1023237',  1.8, 5, 0.48, 0.50, 1.06, '풀 안 물놀이'),
    # **선명도보다 얼굴이 먼저다.** 6.2 구간이 제일 선명했지만(229) 카메라가
    # 옆모습을 아주 가까이 잡아 머리카락과 어깨만 나왔다. 초반은 흐린 대신
    # 얼굴이 있다 — 짧게(2초) 써서 흐린 게 덜 보이게 한다
    ('P1023239',  1.4, 4, 0.46, 0.42, 1.10, 'DJ — 얼굴'),
    ('P1023231',  9.0, 5, 0.50, 0.50, 1.06, '풀에 사람 가득'),
    ('P1023237',  3.2, 4, 0.50, 0.50, 1.06, '튜브 탄 사람들'),
    ('P1023235',  4.6, 5, 0.50, 0.50, 1.05, '풀 + 튜브'),
    ('P1023239',  9.4, 5, 0.50, 0.50, 1.05, 'DJ — 믹서 위의 손'),
    ('P1023234',  5.0, 6, 0.50, 0.50, 1.05, '위에서 본 풀 — 정보가 얹힐 자리'),
]
