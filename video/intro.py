"""
행사 인트로 — AFTER SUNSET. 30fps · 23초 · 120BPM.
...
**그리고 한 방 뒤에 박이 있어야 합니다.** 한 방만 크고 뒤가 무박이면 결국
걸 자리를 못 정합니다. 판 전체가 120BPM 이고 한 방 뒤로 마디 셋을 계속 칩니다 —
디제이는 17.0 에 바로 걸어도 되고, 박에 맞춰 놓고 아무 데서나 들어와도 됩니다.
한 박 0.5초는 30fps 에서 정확히 15프레임이라 화면의 박과 소리의 박이 안 어긋납니다.
...
FPS = 30
BEAT, BAR = 0.5, 2.0                   # 120BPM. 한 박 = 15프레임
...
T_BUILD, T_GO = 13.0, 17.0         # 다시 올리는 구간과 마지막 한 방(마디 첫 박)
T_GAP = T_GO - BEAT                # 소리가 통째로 비는 한 박. 화면도 같이 끈다
T_SAY = 9.50                       # 기계가 BLACKOUT 을 부르는 자리
T_SETTLE = 19.0                    # 화음이 내려오기 시작하는 자리 — 여기서 이름만 남긴다
DUR = T_GO + BAR * 3               # 23.0 — 마디 첫 박에서 끝난다
NBEAT = int(BAR * 3 / BEAT)        # 한 방 뒤에 세는 박의 수
...
def beats(img, k):
    """한 방 뒤에 깔리는 박 표시. **디제이가 눈으로도 박을 잡게 한다.**

    소리만으로 맞추게 두면 언제 끝나는지가 안 보인다. 마디 첫 박은 길고 호박색,
    지나간 박은 남기고 지금 박만 밝다 — 얼마나 남았는지가 한눈에 보인다.
    제일 오른쪽 눈금에 닿는 순간이 파일의 마지막 프레임이다."""
    y = int(H * (0.945 if W > H else 0.960))
    x0, x1 = M, W - M
...
def frame(t, i, A, rng):
    ...
    if t >= T_LOCK:
        ...
        if t >= T_BUILD:
            ...
            for num, t0 in (('3', T_GO - BEAT * 6), ('2', T_GO - BEAT * 4),
                            ('1', T_GO - BEAT * 2)):
                # 0.48 초는 소리보다 빨리 사라져서 숫자가 스치기만 했다.
                # **숫자는 다음 숫자가 올 때까지 남아 있어야 세는 것으로 보인다.**
                q = (t - t0) / 0.88
                if 0 <= q < 1:
    ...
    # **한 방 뒤에는 화면이 흔들리면 안 된다.** 꼬리에 박을 깔면서 매 박 어택이
    # 서게 됐고, 글리치가 그때마다 이름 줄을 밀어 정렬이 어긋난 것처럼 보였다 —
    # 실제로 밑줄과 형식 줄이 여백보다 59px 왼쪽에서 시작하고 있었다.
    # 튀는 건 한 방 직후 0.4초까지만. 그 뒤는 판이 고정이다.
    if 10.4 < t < T_GO + 0.4:
        img = slices(img, hhit if t < T_LOCK else hhit * 0.5, rng)
    img += rng.standard_normal((H, W, 1)).astype(np.float32) * 0.010
    tail = DUR - t
    if tail < 0.30:                       # 마디 첫 박에서 딱 끊는다
