...
# **인트로는 박자가 있어야 한다.** 한 방만 크게 치고 뒤에 셀 것이 없으면
# 디제이가 어디서 걸어야 할지 정할 수 없다 — "노래 시작하기가 애매하다"의 정체다.
# 판 전체를 120BPM 격자에 올린다. 한 박 0.5초는 30fps 에서 정확히 15프레임이라
# 그림과 소리가 **프레임 단위로** 맞는다(128BPM 은 0.46875초라 프레임에 안 떨어진다).
BPM = 120.0
BEAT = 60.0 / BPM                 # 0.5초 = 15프레임
BAR = BEAT * 4                    # 2.0초

T_GO = 17.0                       # 마지막 한 방. **마디 첫 박이다**
T_GAP = T_GO - BEAT               # 한 박을 통째로 비운다
T_SAY = 9.50                      # 기계가 BLACKOUT 을 부르는 자리
T_BUILD = 13.0                    # 다시 올리기 시작하는 자리. 여기도 마디 첫 박
DUR = T_GO + BAR * 3              # 23.0 — **마디 첫 박에서 끝난다**
N = int(DUR * SR)
...
def pip(dur=0.10, f=880.0, gain=1.0):
    """시보음. 방송 시계가 정각 앞에 내는 그 소리다.

    **끝을 알리는 가장 확실한 신호다.** 낮게 세 번, 마지막에 한 번 높게 치면
    "다음 박이 지금"이라는 게 설명 없이 전달된다 — 라디오가 백 년 쓴 방법이고
    기계가 내는 소리라 이 판의 규칙도 안 깬다.
    아주 짧고 각져야 한다. 길면 음이 되고, 음이 되면 세는 소리가 아니다."""
    n, t = _n(dur)
    x = np.sin(2 * np.pi * f * t) + 0.28 * np.sin(2 * np.pi * f * 2 * t)
    e = np.clip(t / 0.0015, 0, 1) * np.clip((dur - t) / 0.012, 0, 1)
    return x * e * gain
...
def build():
    ...
    hm *= 1 - np.clip(1 - np.abs(tt0 - (T_GAP + T_GO) / 2) / 0.26, 0, 1)
    place(b, hm, 1.2)
    HUM_START = 1.2
    ...
    SPOKEN = [('BLACKOUT', T_SAY, 92, 1.42),
              ...
              # 두 박마다 하나. 마지막 ONE 이 끝나는 자리가 곧 정적의 시작이다
              ('THREE',        T_GO - BEAT * 6, 90, 0.92),
              ('TWO',          T_GO - BEAT * 4, 90, 0.92),
              ('ONE',          T_GO - BEAT * 2, 86, 1.02)]
    ...
    while at < 10.0:
        ...
        place(m, relay(0.62 * q, 20 + k), at + BEAT * 0.5)
        if k % 2 == 1:
            place(m, metal(0.5, 0.36 * q, 30 + k, 320), at + BEAT * 0.75)
        at += BEAT
        k += 1
    ...
    place(m, turbine(BAR * 1.75, 40, 132, 0.40, 300, 4200, 36), T_BUILD)   # 끊기지 않는 바닥
    place(b, hum(BAR * 1.75, 60.0, 0.16), T_BUILD)
    RB = T_GAP - BAR * 0.75
    place(m, riser(BAR * 0.75, 0.54, 7), RB)
    place(m, turbine(BAR * 0.75, 70, 210, 0.58, 800, 7500, 34), RB)
    place(m, turbine(BAR * 0.75, 34, 78, 0.34, 260, 1100, 35), RB)   # 밑을 같이 올린다
    place(b, hum(BAR * 0.75, 60.0, 0.10), RB)

    # ── 17.0  마지막 한 방. 마디 첫 박이다 ────────────────
    # **화음을 2.4초 버틴 뒤에 내려온다.** 치자마자 줄어들면 타격이지 벽이 아니다.
    place(m, chord(BAR * 2.7, 1.15, 2.4, 50), T_GO)
    place(m, voxchord(BAR * 2.3, 0.52, 2.2), T_GO + 0.05)
    place(m, metal(2.4, 0.90, 27, 118), T_GO)
    place(m, metal(2.0, 0.55, 37, 76), T_GO + 0.03)
    place(m, bell(BAR * 2.4, 0.62, 228, 8), T_GO)      # 중역을 오래 끌어 준다
    place(m, bell(BAR * 1.9, 0.28, 342, 18), T_GO + 0.04)
    ...
    # ── 17.0–23.0  **디제이가 맞출 박.** 이게 이 판의 결론이다 ──
    # 한 방만 크게 치고 뒤가 무박이면 걸 자리를 못 정한다. 한 방 뒤로 마디 셋 동안
    # 120BPM 을 계속 친다 — 디제이는 이 박에 맞춰 놓고 아무 데서나 들어오면 된다.
    # **세기를 줄이지 않는다.** 줄어드는 박은 못 맞춘다. 끝까지 같은 크기로 간다.
    NB = int(BAR * 3 / BEAT)
    for k in range(1, NB):
        at = T_GO + k * BEAT
        down = (k % 4 == 0)                        # 마디 첫 박은 조금 세게
        # 화음이 물러나는 만큼 박을 **올린다.** 같은 크기로 두면 뒤로 갈수록 묻히고,
        # 묻힌 박은 못 맞춘다 — 실제로 첫 박 대비 끝이 3분의 1까지 내려가 있었다.
        g = 0.26 + 0.30 * (k / NB)
        place(plb, thump(0.30, g * (1.4 if down else 1.0)), at)
        place(plm, relay(g * (1.5 if down else 1.05), 120 + k), at)
        if down:
            place(plm, metal(0.45, g * 0.8, 130 + k, 300), at)

    # 19.0  같은 화음이 한 옥타브 위에서 되받는다. 마디 둘째 첫 박
    place(m, chord(BAR * 1.7, 0.30, 0.7, 70), T_GO + BAR)
    place(f, air(1.6, 0.14, 28), T_GO + BAR)

    # ── 19.0–23.0  기계가 잦아든다 ─────────────────────────
    # **끝이 허전한 건 조용해서가 아니라 아무 일도 안 일어나서다.**
    # 전원이 들어오던 이야기를 거꾸로 돌려 닫는다 — 모터가 서고, 압이 빠지고,
    # 접점이 하나씩 떨어지고, 험만 남는다. 전부 박 위에 놓는다.
    place(m, servo(1.5, 640, 150, 0.26, 20), T_GO + BAR)
    place(m, air(1.2, 0.26, 48), T_GO + BAR * 1.75)
    place(m, bell(2.6, 0.20, 171, 28), T_GO + BAR * 2)
    place(b, hum(BAR * 3, 60.0, 0.13), T_GO + BAR)
    place(m, servo(0.9, 240, 96, 0.14, 18), DUR - BEAT)

    # ── 21.0–23.0  마지막 한 마디는 **카운트인이다** ───────
    # 인트로가 끝나는 프레임에 바로 걸려면, 끝나기 전에 네 박을 세어 줘야 한다.
    # 사람은 반응에 0.2초가 걸려서 "끝났다"를 보고 누르면 이미 늦는다.
    # 낮게 셋, 마지막에 한 번 높게 — 다음 박(23.0)이 걸 자리다.
    CUE = T_GO + BAR * 2
    for j, f0 in enumerate((880, 880, 880, 1320)):
        place(plm, pip(0.085 if j < 3 else 0.11, f0, 0.62 if j < 3 else 0.80),
              CUE + j * BEAT)
    ...
    soft = 1 - 0.32 * np.clip((t_all - T_BUILD) / 0.5, 0, 1)
    m *= 1 - 0.92 * env * soft
    b *= 1 - 0.72 * env * soft
    ...
    # ── 박을 세우는 진짜 방법 ─────────────────────────────
    # 두꺼운 화음 **위에 타격음을 더해서는 박이 안 선다** — 실제로 재 보니
    # 박과 박 사이가 같은 크기였다. 소리를 더하는 대신 **박마다 덜어 낸다.**
    # 화음 전체가 120BPM 으로 숨을 쉬면 디제이는 킥이 없어도 박을 잡는다.
    # 클럽에서 늘 듣는 펌핑이라 귀가 곧바로 알아본다.
    ph = ((t_all - T_GO) % BEAT) / BEAT
    pump = np.where(t_all >= T_GO - 0.02,
                    1 - 0.46 * np.clip(1 - ph / 0.78, 0, 1) ** 1.5, 1.0)
    m *= pump
    b *= pump
    f *= pump
    # 박 자체는 눌리면 안 된다. 눌린 뒤에 얹는다
    m += plm
    b += plb
    ...
    st *= 0.95
    fade = int(0.15 * SR)   # 마디 첫 박에서 딱 끊는다. 길게 빼면 걸 자리가 흐려진다
