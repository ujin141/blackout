...
def horn(dur=2.6, f0=58.0, gain=1.0, seed=6):
    ...
    # 0.22초에 열고 그 뒤로는 쭉 내려간다. 천천히 열되 정점은 앞에 둔다.
    e = np.clip(t / 0.22, 0, 1) ** 1.3 * np.exp(-t * (2.3 / dur))
    return x * e * gain
...
def build():
    ...
    # 16.50–16.75 는 통째로 비운다. 험까지 끊어야 진짜 정적이 된다
    tt0 = np.arange(len(hm)) / SR + 1.2
    hm *= 1 - np.clip(1 - np.abs(tt0 - 16.62) / 0.14, 0, 1)
    place(b, hm, 1.2)
    HUM_START = 1.2
    ...
    # 그 프레임에 정확히 걸 수 있다.
    # 라이저는 16.5 에서 끝난다 — **한 방 앞의 0.25초 정적이 크기를 만든다.**
    # 소리를 키우는 것보다 직전을 비우는 게 훨씬 크게 들린다.
    place(m, riser(1.70, 0.42, 7), 14.80)
    place(m, turbine(1.70, 70, 210, 0.44, 800, 7500, 34), 14.80)
    place(b, hum(1.70, 60.0, 0.10), 14.80)

    # 16.75  마지막 한 방. 뱃고동 두 겹을 5도로 겹쳐 배가 두 척인 것처럼
    place(m, horn(4.2, 38, 1.00, 16), 16.75)
    place(m, horn(3.6, 57, 0.62, 26), 16.82)
    place(m, metal(2.4, 0.90, 27, 118), 16.75)
    place(m, metal(2.0, 0.55, 37, 76), 16.78)
    place(b, boom(3.2, 1.00), 16.75)
    place(f, air(1.4, 0.55, 18), 16.75)
    place(f, riser(0.9, 0.28, 19), 16.75)          # 위로 흩어지는 잔해

    # 18.4  같은 고동이 한 번 되받는다. 여운이 생긴다
    place(m, horn(2.6, 38, 0.16, 36), 18.75)
    place(f, air(1.6, 0.14, 28), 18.75)
    ...
    # 16.50–16.74 를 통째로 0 으로 눌러야 다음 한 방이 두 배로 들린다.
    t_all = np.arange(N) / SR
    gap = np.ones(N)
    gap[(t_all >= 16.46) & (t_all <= 16.74)] = 0.0
    ...
    mix = (m + b * 0.55 + vo * 2.30 + reverb(vo, 1.1, 0.16) * 0.30
           ...
           + reverb(tail_bus, 4.5, 0.55) * 0.45)
    # **존재감 대역을 올린다.** 기계음의 성격은 딸깍·쇳소리처럼 2~6kHz 에 있고,
    # 작은 스피커가 가장 잘 내는 대역도 여기다. 이걸 안 올리면 웅웅거리기만 한다.
    # 잔향까지 끊어야 진짜 정적이다. 버스만 눌러 두면 앞서 친 소리의 잔향이
    # 빈자리를 채워 버린다 — 섞은 뒤에 한 번 더 눌러야 한다.
    mix *= gap

    mix = mix + hp(mix, 1900, 2) * 0.45
