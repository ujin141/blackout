...
#   (이름, 정원, 지금까지 모인 수, 마감일)
# **2차는 30 을 다 못 채우고 날짜로 닫혔다.** 정원을 실제 마감 인원으로
# 내리고 남은 열 자리를 3차로 넘긴다 — 30 을 그대로 두면 '2차 마감' 인데
# 20/30 이라 표가 스스로 모순된다
...
# **지금이 마지막 차수인가.** '마지막' 은 이때만 쓸 수 있는 말이다 —
# 그 전에 쓰면 다음 차수를 열 때 그 말이 그대로 돌아온다.
IS_LAST = bool(OPEN_WAVE) and OPEN_WAVE[0] == WAVES[-1][0]
LAST_LINES = ([f'{OPEN_WAVE[0]}가 마지막입니다',
               f'{OPEN_WAVE[3]} 지나면 예약을 받지 않습니다',
               '현장 판매 없습니다'] if IS_LAST else [])

# **'오늘 자정' 은 마감 당일에만 참이다.** 2차 판을 그대로 3차에 썼더니
# 8/28 마감인데 '오늘 자정' 이라고 나갔다. 올리는 날 이 한 줄만 바꾼다.
DUE_TODAY = False
DUE_STR = '오늘 자정' if DUE_TODAY else (f'{OPEN_WAVE[3]} 자정' if OPEN_WAVE else '')
...
SALE = 'all'      # 8/17 2차 오픈. 그 전에는 'table' 이었다
...
# **이미 열린 차수를 '언제부터' 라고 쓰지 않는다.** 8/17 이 지난 뒤에도
# '2차 신청 8/17부터' 가 판에 남아 있으면 아직 안 열린 것으로 읽힌다.
NEXT_OPEN = ('2차 신청  8/17(월)부터' if SALE == 'table' else '')
...
# **현장에서 실제로 묻는 것만 적는다.** 락커와 흡연은 문 앞에서 매번 나오는
# 질문이라 판에도 들어간다 — 문자로만 안내하면 그날 안 읽고 온 사람이 묻는다
RULES = ('실내 흡연 금지(전자담배 가능)  ·  짐은 1층 락커 이용(유료)  ·  '
         '클럽에 맞지 않는 복장이나 내부 상황에 따라 입장이 불가할 수 있습니다')
