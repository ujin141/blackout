...
크루 여덟 명의 계정이 같이 움직이는 게 이 판의 진짜 값이다. 우리가
...
120BPM, 한 마디 2초. 마디마다 한 명씩 넘긴다.
    ...
    마디 0~1        타이틀. 곡은 코드와 하이햇뿐이라 조용하다
    마디 2~         DJ 한 명씩. 킥이 들어오는 마디 2 에서 첫 명이 뜬다
    마지막 한 마디  날짜 · 장소 · 예약. 필터가 열리고 클랩이 붙는 자리

**길이를 박지 않는다.** 20초로 고정해 뒀다가 XANTHIC 이 라인업에
들어오면서 한 명이 잘렸다 — 사람 수에서 뽑는다.
...
BAR = 2.0                                  # 120BPM
NAMES = EV.LINEUP                          # 타임테이블 순서 그대로
DUR = 2.0 * (2 + len(NAMES) + 1)           # 타이틀 2 + DJ + 아웃트로 1
