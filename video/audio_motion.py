...
촌스러움은 세 가지에서 나온다. 이 파일은 셋 다 반대로 짠다.
    ...
    party   128  멜로딕 하우스. i→VI 두 코드 + 두 마디 모티프. 유일하게 선율이 있는 판
...
def build(style):
    ...
    # ── PARTY — 멜로딕 하우스. 이 파일에서 유일하게 '호감 가는' 판 ──
    # 처음엔 선율을 통째로 뺐다. 뽕짝을 피하려던 건데 **과하게 뺐다** —
    # 뽕짝은 선율이 아니라 **엇박에서 튀는 베이스**에서 온다. 선율이 없으니
    # 속이 빈 소리가 됐고 "이상하다" 는 말을 들었다.
    #
    # 그래서 셋을 되돌린다.
    #   코드 진행  한 코드로 버티면 정지 화면이다. i → VI 두 개면 숨이 생긴다
    #   3음       근음·5도만 쓰면 비어 있다. 마이너 3음이 있어야 정서가 생긴다
    #   짧은 모티프 두 마디짜리 한 가락. 외워지는 게 있어야 다시 듣는다
    # 지키는 것은 하나다 — **베이스는 여전히 킥 밑에 깔려서 지속된다.**
    elif style == 'party':
        F, AB, C5 = R * 8, R * 8 * 1.189, R * 8 * 1.498      # F · Ab · C
        DB, F5 = R * 8 * 0.794, R * 8 * 1.0                  # Db · F
        PROG = [(R, [F, AB, C5]), (R, [F, AB, C5]),          # i
                (R * 0.794, [DB, F5, AB]), (R * 0.794, [DB, F5, AB])]   # VI
        # 두 마디짜리 모티프. 코드음만 밟되 **오르내린다** — 그게 가락이다
        MOTIF = [(0.0, 2), (0.75, 1), (1.5, 0), (2.5, 1), (3.0, 2), (3.5, 1)]
        for b in range(1, bars + 1):
            g = 0.84 if b < DROP else 1.0
            root, ch = PROG[(b - 1) % 4]
            for x in range(4):
                place(kickbus, kick0(0.44, g), T(b, x)); kicks.append(T(b, x))
                place(perc, hat(0.20, 0.20 * g, open_=True), T(b, x + 0.5))
            place(perc, clap(0.40 * g), T(b, 1)); place(perc, clap(0.40 * g), T(b, 3))
            for i in range(8):
                if i % 4 != 2:
                    place(perc, hat(0.04, 0.085 * g), T(b, i * 0.5 + 0.25))
            # **베이스는 코드를 따라가되 마디 내내 이어진다.** 음이 바뀌는 것과
            # 엇박에서 튀는 것은 다르다 — 튀지 않으면 쿵짝이 안 된다
            place(bass, sustain(root, bar * 0.99, 0.58), T(b))
            place(lead, pad(ch, bar * 0.98, 0.13, 2600), T(b))
            if b >= DROP:
                for at, i in MOTIF:
                    place(lead, stab([ch[i]], beat * 0.40, 0.24 * g, 5600, 0.26), T(b, at))
        place(fx, noise_riser(bar * 1.6, 300, 9000, 0.32), T(DROP - 1))
        place(fx, impact(2.2, 0.65), T(DROP))
