...
우진 지시. 얼굴이나 손이 화면을 채우면 **파티가 아니라 인물 사진**이 된다 —
사람이 많아 보이는 게 목적인데 정반대가 된다.

    뺀 것   DJ 손 · DJ 상반신 · 뒷모습 인물 — 셋 다 화면 절반을 한 사람이 먹었다
    줌      1.12 까지 걸던 걸 **1.06 이하**로 낮췄다. 확대가 곧 클로즈업이다

DJ 가 한 컷도 안 나오지만 `push` 의 자막이 여덟 이름을 부른다 —
얼굴을 크게 박는 것보다 그쪽이 사람 수를 더 잘 말한다.
...
CUTS = {
    'push': [
        ('P1023237',  5.0, 5, 1.06, 1.00, 'hook',   '계단·풀 가득 (훅)'),
        ('P1023233',  1.0, 5, 1.00, 1.04, 'filled', '풀 안 물놀이'),
        ('P1023233', 29.0, 5, 1.00, 1.04, 'close',  '루프탑 전경 + 사람들'),
        ('P1023234', 16.0, 6, 1.05, 1.00, 'crew',   '풀 가득'),
        ('P1023234',  9.5, 5, 1.00, 1.05, 'nosale', '풀 가득 — 끝까지 사람'),
        ('P1023234', 41.0, 10, 1.04, 1.00, 'cta',   '하트 네온 + 풀 — 끝 판'),
    ],
    'day': [
        ('P1023235',  3.0, 7, 1.05, 1.00, None,   '풀 · 튜브 (훅)'),
        ('P1023233', 20.0, 7, 1.00, 1.04, None,   '루프탑 + 풀'),
        ('P1023234',  5.5, 7, 1.05, 1.00, None,   '풀 넓게'),
        ('P1023234', 37.0, 7, 1.03, 1.00, 'info', '루프탑 + 사람들 — 정보'),
    ],
    'dusk': [
        ('P1023231',  8.5, 6, 1.06, 1.00, None,   '풀 가득 (훅)'),
        ('P1023234', 24.5, 6, 1.00, 1.04, None,   '풀 + 카바나'),
        ('P1023234', 12.0, 6, 1.05, 1.00, None,   '하트 네온 + 풀'),
        ('P1023235',  0.0, 6, 1.00, 1.04, None,   '풀 · 사람들'),
        ('P1023233', 32.0, 8, 1.03, 1.00, 'info', '핑크 LED + 사람 — 정보'),
    ],
}
...
def check(tag):
    ...
    for c in CUTS[tag]:
        ...
        seen.add(k)
    def span(t, c):
        return c[1], c[1] + c[2] * (60.0 / audio_set.PRESET[t]['bpm'])

    for other in CUTS:
        for a in CUTS[tag]:
            for b_ in (CUTS[other] if other != tag else []):
                if a[0] != b_[0]:
                    continue
                a0, a1 = span(tag, a)
                b0, b1 = span(other, b_)
                assert a1 <= b0 or b1 <= a0, (
                    f'{tag} {a[0]}@{a[1]} 과 {other} {b_[0]}@{b_[1]} 의 '
                    f'구간이 겹친다')
