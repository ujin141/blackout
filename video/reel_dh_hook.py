...
WORDS = [(0.0, 1.2, '내일 밤 10시', '9.26 토 · 추석 마지막 밤'),
         (1.2, 2.4, '압구정', '딥하우즈'),
         (2.4, 3.6, 'DJ 5명', '테크하우스 · 베이스하우스 · 테크노'),
         (3.6, 4.8, '새벽 2시까지', '22:00 — 02:10'),
         (5.2, 6.4, '혼자 와도 됨', '1인 예매 환영'),
         (6.8, 9.0, 'AFTER MOON', '예매 → 프로필 링크')]
...
def main():
    ...
    def frames():
        ...
        for i in range(NF):
            ...
            if t >= 6.8:
                logo(img, M, H * 0.20, 0.30, fade(t, 6.8, 0.3))
                d = plate(DATE, step(3), 0.06)
                put(img, d, (W - d.shape[1]) / 2, H * 0.62, fade(t, 7.0, 0.3))
            out = finish(img)
            if i == 4:
                cover = out
            yield out
