...
python short_card.py ad        **유료 광고판** — 순서가 전환용으로 다시 짜여 있다
"""
import os
import re
import subprocess
...
# **값을 켜고 끄는 건 여기가 아니라 `event.py` 의 `SHOW_PRICE` 다.**
# 사전예약제라 판에 값을 안 적기로 했다. 이 스위치는 광고판에서만 한 번 더
# 막는 용도로 남겨 둔다 — 둘 중 하나라도 꺼져 있으면 값이 안 나간다.
AD_PRICE = True
...
def ad_cards():
    ...
    광고는 처음 보는 사람이고 1초 뒤에 넘긴다 — 그래서 순서가
        ...
        증거     80자리 중 20자리가 찼습니다  남이 이미 샀다
        희소     30자리 남았습니다
        지금     현장 판매 없습니다          지금 안 하면 못 산다
        행동     프로필 링크에서 예약        갈 곳을 정확히 말한다

    **'지금' 칸이 이 판의 심장이다.** 앞의 다섯은 다른 판에도 있지만,
    사람을 오늘 움직이는 건 그 한 줄뿐이다.

    값을 적을 때는 그 자리가 "3차부터 만 원 오릅니다" 였다.
    값을 숨긴 지금은 **"현장에서는 못 산다"** 가 같은 일을 한다 —
    사전예약제라는 사실이 그대로 마감 압박이 되는 구조다.
    """
    ...
    # **칸이 남으면 순서대로 채운다. 같은 말을 두 번 넣지 않는다** —
    # 예전엔 같은 줄을 두 번 밀어 넣어서 '30자리 남았습니다' 가 연달아 나왔다.
    # 값을 숨기면 두 칸이 비는데, 그 자리는 남은 자리와 현장 판매 없음이 받는다
    for col, t in ((AQUA, EV.LEFT_LINE), (DEEP, EV.PRICE_PUSH), (INK, EV.PRICE_LINE)):
        if len(c) >= 7:
            break
        if t not in [x[1] for x in c]:
            c.insert(-1, (col, t))
    return c[:7]
...
def sale_cards():
    ...
    c.append((INK, '양재 루프탑'))
    # 값을 숨기면 두 칸이 빈다 — 사전예약제라는 사실이 그 자리를 받는다.
    # **지금 사야 할 이유가 값 인상에서 "현장에서는 못 산다" 로 바뀐다**
    for col, t in ((CORAL, EV.PRICE_PUSH), (DEEP, EV.PRICE_LINE)):
        if len(c) >= 7:
            break
        if t not in [x[1] for x in c]:
            c.insert(2, (col, t))
...
def render(mode='intro'):
    ...
    for i in range(nf):
        ...
        # **글자 뒤에 검은 띠를 두르지 않는다.** 띠는 그 자체로 얹은 물건으로
        # 읽힌다 — 대신 화면 아래를 통째로 떨어뜨린다. 릴스에서 흔히 쓰는
        # 색보정이라 라벨이 아니라 '분위기' 로 읽히고, 자막도 같이 산다.
        if kind != 'card':
            yy = np.arange(H, dtype=np.float32)[:, None, None]
            # 자막이 앉는 높이(0.69)에서 이미 다 떨어져 있어야 한다 —
            # 바닥에서야 어두워지면 밝은 튜브 위 글자는 그대로 묻힌다
            img *= 1 - 0.56 * np.clip((yy - H * 0.50) / (H * 0.20), 0, 1) ** 1.15

        # **상태는 처음부터 끝까지 붙어 있다.** 스크롤로 지나가는 영상이라
