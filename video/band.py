...
def tier_chip(u, word, n, bg, fg):
    ...
    wm = tmask_bl(word, BRAND, 25, 0.26)
    bw, bg_gap = HAIR * 3, 11
...
def draw_unit(u, bg, fg, accent, word, n):
    ...
    name = tmask_bl(EV.NAME, BRAND, 46, 0.10)
    paint_bl(u, name, x, BL1, color=fg)
    fmt = tmask_bl(EV.FORMAT, BRAND, 21, 0.10)
    ...
    # ── 가운데 오른쪽 · 날짜·시간 ─────────────────────────
    rx = chip_x - 44
    d1 = tmask_bl(DATE_SHORT, BRAND, 30, 0.14)
    d2 = tmask_bl(TIME_SHORT, BRAND, 21, 0.18)
