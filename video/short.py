...
# **한 줄을 2.3초씩 붙여 두면 루즈하다.** 다섯 줄로 26박을 나눴더니 글자가
# 화면에 붙어 있는 시간이 컷보다 길어서 영상이 멈춘 것처럼 보였다.
# 두세 박(0.9~1.4초)으로 끊고 줄을 늘린다 — 문장을 쪼개면 같은 말인데 굴러간다.
# **정보를 순서대로 나열하면 재미가 없다.** '서울 → 양재 → 날짜 → 풀파티 →
# 솔로파티' 는 목록이라 다 읽을 이유가 없다. 두 가지를 바꾼다.
#   숫자를 준다      "디제이 일곱 명", "9시 반부터" — 막연한 말보다 훨씬 걸린다
#   구조를 설명한다  솔로파티가 뭔지가 이 행사에서 제일 궁금한 대목이다.
#                    "혼자 온 사람들끼리 섞여요" 가 '솔로파티' 라는 낱말보다 세다
CAPS = [(0, 3, '여기 서울이에요'),
        (3, 6, '양재에 이런 데가 있어요'),
        (6, 9, '8월 29일 토요일'),
        (9, 12, '디제이 일곱 명이 돌아가면서'),
        (12, 15, '9시 반부터는 솔로파티'),
        (15, 18, '혼자 온 사람들끼리 섞여요'),
        (18, 21, '아는 사람 없이 와도 돼요'),
        (21, 24, '끝나면 신사 ACE로 2차'),
        (24, 26, '거기 테이블도 빼놨어요')]
...
def render(style='deep'):
    ...
    for i, (key, fno, ox, z) in enumerate(plan):
        ...
        for b0, b1, txt in CAPS:
            if b0 <= b < b1:
                k = np.clip((b - b0) / 0.16, 0, 1)      # 박 위에서 딱 붙는다
                cy = H * 0.615
        ...
        if b >= tail:
            ...
            if k2 > 0.004:
                ...
                # 협업 네 곳. **작게, 맨 아래** — 이름은 남기되 CTA 를 안 가린다
                paint(img, tmask('with', BRAND, 20, 0.30), W / 2, yb + H * 0.098,
                      color=PAPER, a=float(k2) * 0.55, anchor='c')
                # **폭에 맞춰 크기를 뽑는다.** 21px 로 박아 뒀더니 네 곳 이름이
                # 화면을 넘어 오른쪽이 잘렸다 — 협업사 이름이 잘리면 안 넣느니만 못하다
                ps = min(21, fit(EV.PARTNERS_STR, BRAND, W * 0.88, 0.16))
                paint(img, tmask(EV.PARTNERS_STR, BRAND, ps, 0.16), W / 2, yb + H * 0.130,
                      color=PAPER, a=float(k2) * 0.72, anchor='c')
