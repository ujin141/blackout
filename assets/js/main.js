...
const EVENT = {
  on: true,
  // 예매는 파티모아에서 받는다. **파티를 공개로 바꾼 뒤** 그 파티 주소로
  // 바꿀 것 — 초안(draft) 상태에서는 그 주소가 아직 안 열린다
  form: 'https://www.partymoa.com/',              // 사전예약
  date: '2026-09-26',                             // 행사 당일 — D-day 를 여기서 센다
  until: '2026-09-26',                            // 이 날이 지나면 스스로 안 뜬다
  delay: 1200,                                    // 들어오자마자 덮치면 그냥 닫는다
  muteHours: 20,                                  // '오늘 하루 안 보기' 가 유지되는 시간
  cap: 30,                                        // 정원
  done: 0,                                        // 지금까지 예약된 수 — 여기만 고치면 막대가 따라온다
  /* 지금 받는 차수. **여기만 고치면 팝업과 파티 섹션이 같이 따라옵니다.**
     마감일이 지나면 그 줄은 스스로 사라집니다 — 다음 차수 값으로 바꾸세요. */
  wave: { name: '1차', nameEn: '1st', left: 30, close: '2026-09-25' }
};
