...
| 빠진 것 | 왜 | 어떻게 |
|---|---|---|
| `AROS/` `Lynn/` `TS/` `Akill/` `V/` — 멤버 원본 사진 (약 5MB) | 개인정보(프로필 카드에 전화번호)가 있어 공개 저장소에 올리지 않음 | **USB·드라이브로 직접 옮기세요.** 누끼 결과물(`assets/img/members/*.webp`)은 저장소에 있으므로, 사진을 다시 따야 할 때만 필요합니다 |
